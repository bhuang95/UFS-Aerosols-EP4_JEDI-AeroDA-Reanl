...
while [ ${IMEM} -le ${ENSED} ]; do
    ...
    BKGDIR=${ROTDIR}/${CDUMP}.${GYMD}/${GH}/${MEMSTR}/${COMP_BKG}/
    ANLDIR=${ROTDIR}/${CDUMP}.${CYMD}/${CH}/${MEMSTR}/${COMP_ANL}/
    [[ ! -d ${BKGDIR} ]] && mkdir -p ${BKGDIR}
    [[ ! -d ${ANLDIR} ]] && mkdir -p ${ANLDIR}
    BKGFILE=${BKGDIR}/${CDUMP}.t${GH}z.atmf${FHR}.nc 
    INCFILE=${ANLDIR}/${CDUMP}.t${CH}z.ratminc.nc
    ANLFILE=${ANLDIR}/${CDUMP}.t${CH}z.ratmanl.nc
