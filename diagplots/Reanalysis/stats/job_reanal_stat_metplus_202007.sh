...
#SBATCH --output=/scratch2/BMC/gsd-fv3-dev/MAPP_2018/bhuang/JEDI-2020/JEDI-FV3/miscLog/metplus_reanl_202007.out
...
#
SDATE=${SDATE:-"2020120100"}
EDATE=${EDATE:-"2021022818"}
INC_H=${INC_H:-"6"}
...
INPUTBASE=${INPUTBASE:-"/scratch2/BMC/gsd-fv3-dev/bhuang/expRuns/UFS-Aerosols_RETcyc/AeroReanl/"}
EXPRUNS="
...
for EXPRUN in ${EXPRUNS}; do
    FCSTPATH=$INPUTBASE/${EXPRUN}/dr-data-backup//GriddedReanl
    FCST_NAME=${EXPRUN}
    if ( echo ${FCST_NAME} | grep "AeroDA" ); then
        ANLFILE="YES"
    else
        ANLFILE="NO"
    fi
...
while [ ${FHR} -le ${FHMAX} ]; do
    ...
    OUTPUTDIR=${INPUTBASE}/${EXPRUN}/diagplots/NASA-EC-AODReanl/PLOTS-${SDATE}-${EDATE}/${OBSTYPE}/
	...
	if [ ${ANLFILE} == "YES" ]; then
            ln -sf $FCSTPATH/${CY}/${CY}${CM}/${CY}${CM}${CD}/NARA-2.0_AOD_${CDATE}.nc ./fcst/${FCST_HEAD}${CDATE}${FCST_SUFF}
	else
            ln -sf $FCSTPATH/${CY}/${CY}${CM}/${CY}${CM}${CD}/fv3_aod_LUTs_fv_tracer_${CDATE}_ll.nc ./fcst/${FCST_HEAD}${CDATE}${FCST_SUFF}
	fi
