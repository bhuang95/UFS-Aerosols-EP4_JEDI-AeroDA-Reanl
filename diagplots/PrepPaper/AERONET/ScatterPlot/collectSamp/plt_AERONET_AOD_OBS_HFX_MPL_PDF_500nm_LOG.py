...
def plot_scatter_density(obs, nodabckg, dabckg, daanal, xmax, bmax, cycs, cyce, pmonth):
    ...
    xlabstr='AERONET ln(AOD) at 500 nm'
    ylabstr='Modeled ln(AOD) at 500 nm'
    for ipt in range(3):
        if ipt==0:
...
def plot_mpl_scatter_density(obs, nodabckg, dabckg, daanal, nbins, xmin, xmax, bmax, cycle, outpre, pmonth, cmap):
    ...
    datamax=max(obs.max(), nodabckg.max(), dabckg.max(), daanal.max())
    datamin=max(obs.min(), nodabckg.min(), dabckg.min(), daanal.min())
    print("HBO: ", datamin, datamax)
    axis=np.linspace(datamin,datamax,nbins+1)
    #axis=np.exp(axis)
    ...
    #if pmonth:
    ptitle=f"500nm Aerosol Optical Depth (AOD) wrt {outpre} \n aggregated in {cycle} over Year 2018-2022"
    #fig=plt.figure(figsize=[20,8])
    fig=plt.figure(figsize=[10,4])
    xlabstr=f"{outpre} ln(AOD) at 500 nm"
    ylabstr='Modeled ln(AOD) at 500 nm'
    for ipt in range(3):
        if ipt==0:
            obs=obs
        ...
        cn=ax.contourf(axis[:-1],axis[:-1],hist2d.swapaxes(0,1),vmin=vmin,vmax=vmax,levels=256,cmap=cmap)#,norm=norm)
        #sc=plt.scatter(obs, hfx, c=z, s=1., cmap='jet', marker='o', vmin=0, vmax=bmax, )

        plt.plot([-5.0, 2.0],[-5.0, 2.0], color='gray', linewidth=2, linestyle='--')
        plt.xlim(-5.0, 2.0)
        plt.ylim(-5.0, 2.0)
    ...
    #fig.tight_layout()
    outfig=f"{outpre}_{cycle}_scatter_LOG2.png"
...
if __name__ == '__main__':
    ...
    vinds=np.where((dabkg_obs>=xmin) & (dabkg_obs<=xmax) &
                   (dabkg_hfx>=xmin) & (dabkg_hfx<=xmax) & 
                   (daanl_hfx>=xmin) & (daanl_hfx<=xmax) & 
                   (freerun_hfx>=xmin) & (freerun_hfx<=xmax))
    ...
    plot_mpl_scatter_density(np.log(obsarr),np.log(freerunarr), np.log(dabkgarr), np.log(daanlarr), nbins, xmin, xmax, bmax, cycle, outpre, pmonth, white_gist_earth) #white_jet)
