...
def plot_monthlymean_yearlymean(xdata, ydata1, ydata2, xlab, ylab, yran, ptitle, pname):
    fsize = 14
    fig = plt.figure(figsize=[16,4])
    ax=fig.add_subplot(111)
    #ax.set_title(f'{plttit}', fontsize = fsize)
    #for iv in range(nvars):
    nyears=int(len(xdata)/12);
    print(nyears)
    plt.plot(xdata, ydata1, '-bo', label='FreeRun', linewidth=2.5)
    plt.plot(xdata, ydata2, '-ro', label='Reanalysis', linewidth=2.5)
    ...
    plt.plot(xdata, m1, '--b', linewidth=2)
    plt.plot(xdata, m2, '--r', linewidth=2)
    yymax=max([max(m1), max(m2)])
    yymin=min([min(m1), min(m2)])
    ...
    for iyear in range(0,nyears):
        ...
        print(ist)
        print(ied) 
        xdatatmp = [xdata[i] for i in range(ist,ied)]
    #    print(xdatatmp)
    #    ydata1tmp = [ydata1[i] for i in range(ist,ied)]
    #    ydata2tmp = [ydata2[i] for i in range(ist,ied)]
    #    mtmp1=sum(ydata1tmp)/12.0
    #    mtmp2=sum(ydata2tmp)/12.0
    #    m1=[mtmp1]*12
    #    m2=[mtmp2]*12
    #    
    #    plt.plot(xdatatmp, m1, '--b', linewidth=2)
    #    plt.plot(xdatatmp, m2, '--r', linewidth=2)
    #    yymax=max([max(m1), max(m2)])
    #    yymin=min([min(m1), min(m2)])

        plt.plot([xdatatmp[0], xdatatmp[0]], [yymax+1, yymin-1], '-k', linewidth=3)
        if iyear == nyears-1:
            plt.plot([xdatatmp[-1], xdatatmp[-1]], [yymax+1, yymin-1], '-k', linewidth=3)
...
if __name__ == '__main__':
   ...
   pname=f'{r2}.png'
   yran=[0.0, 0.8]
   plt1=plot_monthlymean_yearlymean(xdata, ydata1, ydata2, xlab, ylab, yran, ptitle, pname)
